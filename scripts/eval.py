...
test_a = reader.read_field('u')[:, ::sub, ::sub,
                                indent:T_in * 4:4]  #([0, T_in])
test_u = reader.read_field('u')[:, ::sub, ::sub, indent + T_in * 4:indent +
                                (T + T_in) * 4:sub_t]  #([T_in, T_in + T])
...
path = 'eval'
scipy.io.savemat('pred/' + path + '.mat',
                 mdict={
                     'pred': pred.cpu().numpy(),
                     'u': test_u.cpu().numpy()
                 })
