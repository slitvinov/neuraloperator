...
class SimpleBlock2d(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, modes1, modes2, width):
        ...
        self.width_list = [
            width * 2 // 4, width * 3 // 4, width * 4 // 4, width * 4 // 4,
            width * 5 // 4
        ]
        self.size_list = [
            64,
        ...
        self.conv0 = SpectralConv2d(self.width_list[0] + self.grid_dim,
                                    self.width_list[1], self.modes1 * 4 // 4,
                                    self.modes2 * 4 // 4)
        self.conv1 = SpectralConv2d(self.width_list[1] + self.grid_dim,
                                    self.width_list[2], self.modes1 * 3 // 4,
                                    self.modes2 * 3 // 4)
        self.conv2 = SpectralConv2d(self.width_list[2] + self.grid_dim,
                                    self.width_list[3], self.modes1 * 2 // 4,
                                    self.modes2 * 2 // 4)
        self.conv3 = SpectralConv2d(self.width_list[3] + self.grid_dim,
                                    self.width_list[4], self.modes1 * 1 // 4,
                                    self.modes2 * 1 // 4)
        self.w0 = nn.Conv1d(self.width_list[0] + self.grid_dim,
                            self.width_list[1], 1)
        self.w1 = nn.Conv1d(self.width_list[1] + self.grid_dim,
                            self.width_list[2], 1)
        self.w2 = nn.Conv1d(self.width_list[2] + self.grid_dim,
                            self.width_list[3], 1)
        self.w3 = nn.Conv1d(self.width_list[3] + self.grid_dim,
                            self.width_list[4], 1)
    ...
    def forward(self, x):
        ...
        x = torch.cat((x, grid.permute(0, 2, 3, 1).repeat([1, 1, 1, 1])),
                      dim=-1)
        ...
        x2 = self.w0(
            x.view(batchsize, self.width_list[0] + self.grid_dim,
                   size_list[0]**2)).view(batchsize, self.width_list[1],
                                          size_list[0], size_list[0])
        ...
        x2 = self.w1(
            x.view(batchsize, self.width_list[1] + self.grid_dim,
                   size_list[1]**2)).view(batchsize, self.width_list[2],
                                          size_list[1], size_list[1])
        ...
        x2 = self.w2(
            x.view(batchsize, self.width_list[2] + self.grid_dim,
                   size_list[2]**2)).view(batchsize, self.width_list[3],
                                          size_list[2], size_list[2])
        ...
        x2 = self.w3(
            x.view(batchsize, self.width_list[3] + self.grid_dim,
                   size_list[3]**2)).view(batchsize, self.width_list[4],
                                          size_list[3], size_list[3])
...
path = 'KF_vel_N' + str(ntrain) + '_k' + str(loss_k) + '_g' + str(
    loss_group) + '_ep' + str(epochs) + '_m' + str(modes) + '_w' + str(width)
...
train_a = data[:ntrain,
               T_in - 1:T_out - 1, ::sub, ::sub, :].permute(0, 2, 3, 1, 4)
...
train_a = torch.cat((gridx.repeat(
    [ntrain, 1, 1, T, 1]), gridy.repeat([ntrain, 1, 1, T, 1]), train_a),
                    dim=-1)
test_a = torch.cat((gridx.repeat(
    [ntest, 1, 1, T, 1]), gridy.repeat([ntest, 1, 1, T, 1]), test_a),
                   dim=-1)
...
for ep in range(epochs):
    ...
    for xx, yy in train_loader:
        ...
        for t in range(0, T):
            ...
            out = model(x)
            loss = hsloss(out.reshape(batch_size, S, S, out_dim),
                          y.reshape(batch_size, S, S, out_dim))
            train_l2 += loss.item()
    ...
    with torch.no_grad():
        for xx, yy in test_loader:
            ...
            for t in range(0, T):
                ...
                out = model(x)
                test_l2 += lploss(out.reshape(batch_size, S, S, out_dim),
                                  y.reshape(batch_size, S, S, out_dim)).item()
                test_l2_hp += hsloss(out.reshape(batch_size, S, S, out_dim),
                                     y.reshape(batch_size, S, S,
                                               out_dim)).item()

    t2 = default_timer()
    scheduler.step()
    print(ep, t2 - t1, train_l2 / ntrain / T, test_l2_hp / ntest / T,
          test_l2 / ntest / T)
